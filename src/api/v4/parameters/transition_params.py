...
class TransitionParameter(BaseParam):
    """
    Class for the working parameter mu. 
    """


class TransitionWorkingParamSet(BaseWorkingParamSet):
    """
    Class for the set of working parameters for the mu vals. 
    """

    transition_list: list[list[TransitionParameter]]  
    name : str = eqx.field(static=True, default="transition")

    def __init__(self, working_params: jax.Array):
        trans_list = []
        for row in working_params:
            trans_list.append([TransitionParameter(val=working_param) for working_param in row])
        self.transition_list = trans_list 


    def from_natural_params(self, natural_params: jnp.ndarray) -> "TransitionWorkingParamSet":
        """
        Creates a new instance of the working param set from the natural params. 
        """

        trans_list = []
        for row in natural_params:
            

        return TransitionWorkingParamSet(working_params=working_params)
